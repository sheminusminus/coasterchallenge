/*====================================================	
	we'd planned on using snapsvg heavily, and ended up only using it in part of this file 
	it's only job is essentially to ajax in the dinosaur...
	by the time i realized this, it was so integrated that i was worried to change things
	but i'm sure it's not too bad if snap is effecting your load times.
====================================================*/
...
//~~~~~~~~~~~~~~~~~~
// illustrator sometimes appends _#_ to ids for mysterious reasons
//~~~~~~~~~~~~~~~~~~
function sel(s, name) {
	if (name[0] != '#') { name = '#' + name; }
...
function cart(e,L,R,dist,track,x) {
	var Lpos = on(track, x);
	var Rpos = on(track, x + dist);
	var rot = V2rot(Lpos,Rpos);
	Lpos[0] -= L[0];
	Lpos[1] -= L[1];
	return Xform(e, Lpos, rad2deg(rot), L);
}
...
$(document).ready(function() {
	...
	//~~~~~~~~~~~~~~~~~~
	// donorsaur: loaded via snapSVG
	//~~~~~~~~~~~~~~~~~~
	
	var root;
	// decide what the root should be based on screen width
	if (sw < 760) {
		root = Snap('#fullscene-m');
	} else {
		root = Snap('#fullscene-l');
	}
	
	Snap.load('images/donorsaur.svg', function(scene) {
		...
		var R = V2(wheelR);
		var dist = V2dist(L,R);
		var trackLen = track.getTotalLength();
